...
| 07 | En la aplicación de gestión de alumnos | Se hace una solicitud POST a la ruta /Inscripciones de la API | Debería ser posible crear una nueva inscripción en la base de datos a través de la API. La API debe validar los datos proporcionados de acuerdo con las reglas definidas en la entidad "Inscripciones". Si los datos son válidos, la API debe realizar lo siguiente:
   ...
   - El cuerpo de la respuesta debe contener una representación de la nueva inscripción creada.
   - En la base de datos, debe registrarse la creación de la nueva inscripción con un ID asignado.
Si los datos son inválidos o la creación falla, la API debe manejar la situación de la siguiente manera:
   - Responder con un código de estado 400 (Solicitud incorrecta) o 422 (Entidad no procesable) si los datos proporcionados son inválidos o incompletos.
   - La respuesta debe incluir detalles sobre los errores de validación en el cuerpo de la respuesta para que el cliente pueda corregirlos.
   - No se debe realizar ninguna operación en la base de datos si los datos son inválidos.
La API debe ser capaz de manejar errores y responder con un código de estado adecuado en caso de fallos de servidor.
