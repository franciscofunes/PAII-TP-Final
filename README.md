...
| 03 | En la aplicación de gestión de alumnos | Se hace una solicitud POST a la ruta /Alumnos de la API | Debería ser posible crear un nuevo estudiante en la base de datos mediante la API. La API debe validar los datos proporcionados según las reglas definidas en la entidad. Si los datos son válidos:
La API debe devolver una respuesta exitosa con un código de estado 201 (Created).El cuerpo de la respuesta debe contener una representación del nuevo estudiante creado. La base de datos debe reflejar la creación del nuevo estudiante con un ID asignado.Si los datos son inválidos o la creación falla:
La API debe devolver un código de estado 400 (Solicitud incorrecta) o 422 (Entidad no procesable) si los datos proporcionados son inválidos o incompletos.
