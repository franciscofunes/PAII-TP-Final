...
| 06 | En la aplicación de gestión de alumnos | Desarrollo y pruebas de los métodos del servicio de la entidad Alumnos | Deben crearse unit tests para cada método del servicio que maneja la lógica del negocio de la entidad Alumnos. Cada unit test deberá validar que el método funciona correctamente y cumple con su funcionalidad específica. Los unit tests deben cubrir diferentes escenarios, incluyendo casos de éxito y situaciones de error. Deberán verificarse las validaciones, reglas de negocio y lógica de cada método, y asegurarse de que se manejen adecuadamente los casos límite. En caso de que un unit test falle, se deberá investigar y corregir el problema en el código del servicio. La creación y ejecución exitosa de unit tests garantizará la calidad y confiabilidad de la lógica de negocio de la entidad Alumnos en la aplicación.
| 07 | En la aplicación de gestión de alumnos | Se hace una solicitud POST a la ruta /Inscripciones de la API | Debería ser posible crear una nueva inscripción en la base de datos a través de la API. La API debe validar los datos proporcionados de acuerdo con las reglas definidas en la entidad "Inscripciones". Si los datos son válidos, la API debe realizar lo siguiente: Responder con un código de estado 201 (Created). El cuerpo de la respuesta debe contener una representación de la nueva inscripción creada. En la base de datos, debe registrarse la creación de la nueva inscripción con un ID asignado. Si los datos son inválidos o la creación falla, la API debe manejar la situación de la siguiente manera: Responder con un código de estado 400 (Solicitud incorrecta) o 422 (Entidad no procesable) si los datos proporcionados son inválidos o incompletos. La respuesta debe incluir detalles sobre los errores de validación en el cuerpo de la respuesta para que el cliente pueda corregirlos. No se debe realizar ninguna operación en la base de datos si los datos son inválidos. La API debe ser capaz de manejar errores y responder con un código de estado adecuado en caso de fallos de servidor.
| 08 | En la aplicación de gestión de alumnos | Se hace una solicitud GET a la ruta /Inscripciones de la API | Debería recibir una respuesta exitosa con un código de estado 200 (OK) si la solicitud se procesa correctamente y la lista de inscripciones se devuelve según lo especificado. La respuesta incluirá una lista de objetos JSON, donde cada objeto representa una inscripción. Cada objeto de inscripción en la lista contendrá al menos la siguiente información: ID de la inscripción, ID del alumno asociado, Estado de la inscripción y otros campos relevantes según sea necesario. La lista de inscripciones estará ordenada de alguna manera para facilitar su visualización y búsqueda. Si no hay inscripciones en la base de datos, la API deberá devolver una lista vacía. Si ocurren errores al procesar la solicitud, como un problema interno en el servidor o una solicitud mal formada, la API deberá devolver el código de estado apropiado (por ejemplo, 500 Error interno del servidor o 400 Solicitud incorrecta) junto con mensajes de error descriptivos para ayudar a identificar y solucionar el problema. La API debe manejar de manera adecuada cualquier situación inesperada y proporcionar información útil en caso de errores. 
| 09 | En la aplicación de gestión de alumnos | Se hace una solicitud PUT a la ruta `/Inscripciones/{ID}` de la API | Debería ser posible modificar la información de una inscripción existente identificada por su ID. La solicitud PUT deberá incluir los datos actualizados de la inscripción en formato JSON. La API deberá validar y procesar la solicitud correctamente, actualizando los campos especificados de la inscripción en la base de datos. Se debe devolver una respuesta exitosa con un código de estado 200 (OK) o un código de estado 204 (No Content) si la actualización se realiza con éxito. La API deberá manejar errores y devolver un código de estado adecuado en caso de fallos, como 404 (No encontrado) si no se encuentra la inscripción con el ID proporcionado o 400 (Solicitud incorrecta) si los datos proporcionados son inválidos o incompletos. La actualización de campos debe reflejarse correctamente en la base de datos y en futuras solicitudes GET para obtener información actualizada de la inscripción. La API debe ser capaz de manejar errores y responder con un código de estado adecuado en caso de fallos de servidor. La respuesta deberá incluir detalles sobre los errores de validación en el cuerpo de la respuesta para que el cliente pueda corregirlos. No se debe realizar ninguna operación en la base de datos si los datos son inválidos. |
| 10 | En la aplicación de gestión de alumnos | Desarrollo y ejecución de pruebas unitarias para la capa de servicios de la entidad débil Inscripción | Deberán crearse pruebas unitarias para los distintos métodos utilizados por el controlador que interactúan con la capa de servicios de la entidad Inscripción. Cada prueba unitaria deberá validar que el método correspondiente de la capa de servicios funcione correctamente y cumpla con su funcionalidad específica. Las pruebas deberán cubrir diferentes escenarios, incluyendo casos de éxito y situaciones de error. Deberán verificarse las validaciones, reglas de negocio y lógica de cada método, asegurándose de que se manejen adecuadamente los casos límite. En caso de que una prueba unitaria falle, se deberá investigar y corregir el problema en el código de la capa de servicios. La creación y ejecución exitosa de pruebas unitarias garantizará la calidad y confiabilidad de la lógica de negocio de la entidad Inscripción en la aplicación. |
| 11 | En la aplicación de gestión de alumnos | Desarrollo de un nuevo endpoint para borrar una inscripción | Se debe crear un nuevo endpoint en la API que permita eliminar una inscripción existente. La solicitud DELETE deberá ser enviada a la ruta `/Inscripciones/{ID}`. La API deberá validar la existencia de la inscripción con el ID proporcionado antes de intentar eliminarla. Si la inscripción existe, deberá eliminarse de la base de datos y la API deberá responder con un código de estado 204 (No Content) indicando que la eliminación se realizó con éxito. Si la inscripción no se encuentra (ID no existe), la API deberá responder con un código de estado 404 (Not Found) y un mensaje indicando que la inscripción con el ID especificado no fue encontrada. La API deberá manejar adecuadamente cualquier situación inesperada y responder con un código de estado apropiado en caso de fallos de servidor. |
